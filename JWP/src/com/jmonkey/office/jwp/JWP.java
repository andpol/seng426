...
/**
 * This is the controlling class of the JWP word processor application, and
 * includes the application's "main" method.
 */
public class JWP extends JFrame implements ActionListener {

	private static final int[] rv = { 1, 0 };
	private static final String rbn = "com.jmonkey.office.jwp.JWPResources";
	...
	protected final ActionListener m_openWindowAction = new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			DocumentFrame frame = getDesktopManager().getOpenDocument(e.getActionCommand());
	...
	protected final class SFTAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() instanceof JCheckBoxMenuItem) {
				m_parent.getFormatToolBar().setVisible(!m_parent.getFormatToolBar().isVisible());
				((JCheckBoxMenuItem) e.getSource()).setState(m_parent.getFormatToolBar()
						.isVisible());
			}
		}
	}
	...
	protected final class SLTAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() instanceof JCheckBoxMenuItem) {
				m_parent.getFileToolBar().setVisible(!m_parent.getFileToolBar().isVisible());
				((JCheckBoxMenuItem) e.getSource()).setState(m_parent.getFileToolBar().isVisible());
			}
		}
	}

	protected class ColourActionCellRenderer extends JLabel implements ListCellRenderer {
	...
	protected class FontActionCellRenderer extends JLabel implements ListCellRenderer {
	...
	protected class FSActionCellRenderer extends JLabel implements ListCellRenderer {
	...
	protected final class PopupPropertyAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			PropertySheetDialog.display(getMain(), getRegistry().referenceGroup("POPUP"));
			getMain().repaint();
		}
	}
	...
	protected final class MainPropertyAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			PropertySheetDialog.display(getMain(), getRegistry().referenceGroup("MAIN"));
			getMain().repaint();
		}
	}
	...
	protected final class PrintAction extends AbstractAction {
		public PrintAction() {
			super("Print...");
			boolean enabled = getRegistry().getBoolean("MAIN", "print.document.enabled");
			setEnabled(enabled);
		}

		public void actionPerformed(ActionEvent e) {
			Code.event(e);
			JOptionPane.showMessageDialog(getMain(), "Print Not Implemented!", "Not Implemented!",
	...
	protected final void switchedDocumentFrame(DocumentFrame frame, boolean textSelected) {
		setTitle("Main - [" + frame.getTitle() + (frame.getEditor().isChanged() ? "] *" : "]"));
	}
	...
	private JMenuBar createMenuBar() {
		...
		JCheckBoxMenuItem showhideFileToolBar = new JCheckBoxMenuItem("Show File Toolbar");
		...
		JCheckBoxMenuItem showhideFormatToolBar = new JCheckBoxMenuItem("Show Format Toolbar");
	...
	public void doExit() {
		...
		reg.setProperty("USER", "show.file.toolbar", getFileToolBar().isVisible());
		reg.setProperty("USER", "show.format.toolbar", getFormatToolBar().isVisible());
		...
		for (int p = 0; p < m_fileHistory.getItemCount(); p++) {
			reg.setProperty("FILE_HISTORY", m_fileHistory.getItem(p).getText(), m_fileHistory
					.getItem(p).getActionCommand());
		}
	...
	protected final JToolBar getFormatToolBar() {
		if (this.ftb == null) {
			...
			while (colourEnum.hasMoreElements()) {
				...
				try {
					colours.addItem(eam.getColourAction(colourKey,
			...
			if (pos.equalsIgnoreCase(BorderLayout.WEST) || pos.equalsIgnoreCase(BorderLayout.EAST)) {
	...
	public final Registry getRegistry() {
		if (m_mainRegistry == null) {
			try {
				...
				reg.setProperty("USER", "user.name", System.getProperty("user.name"));
				reg.setProperty("USER", "user.timezone", System.getProperty("user.timezone"));
				reg.setProperty("USER", "user.home", System.getProperty("user.home"));
				reg.setProperty("USER", "temp.directory", System.getProperty("java.io.tmpdir"));
				...
				if (reg.sizeOf("MAIN") == 0) {
					reg.initGroup("MAIN",
							new String[][] {
									...
									{ "main.look&feel", UIManager.getSystemLookAndFeelClassName(),
											"String" },
									{ "new.document.title", "New Document", "String" },
									{ "default.content.type", Editor.VALID_CONTENT_TYPES[2],
											"String" } });
				}
				...
				if (reg.sizeOf("COLOURS") == 0) {
					reg.initGroup(
							"COLOURS",
							new String[][] { { "White", Format.colorToHex(Color.white) },
...
//		}
		...
		fileToolPanel.add(getFileToolBar(), reg.getString("USER", "position.file.toolbar"));

		JPanel formatToolPanel = new JPanel();
		formatToolPanel.setLayout(new BorderLayout());
		formatToolPanel.add(getFormatToolBar(), reg.getString("USER", "position.format.toolbar"));
