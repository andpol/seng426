...
/**
 * This class implements a Dialog for selecting a font, consisting of a font
 * family, a style and a font size.
 */
public final class FontChooser extends JDialog implements ActionListener, ListSelectionListener,
		ItemListener {
	private JWP m_app;
	 ...
	 */
	public FontChooser(JFrame owner, String title, boolean modal, AttributeSet initial) {
	...
	public int getFontStyle() {
		return m_plainBox.isSelected() ? Font.PLAIN
				: ((m_boldBox.isSelected() ? Font.BOLD : 0) + (m_italicBox.isSelected() ? Font.ITALIC
						: 0));
	}
	...
	public void itemStateChanged(ItemEvent iEv) {
		...
		if (iEv.getItem() == m_plainBox && iEv.getStateChange() == ItemEvent.DESELECTED) {
			m_boldBox.setEnabled(true);
			m_italicBox.setEnabled(true);
		}
		paintPreviewArea(true);
	...
	private void paintPreviewArea(boolean repaint) {
		if (m_prevArea != null) {
			...
			int style = m_plainBox.isSelected() ? Font.PLAIN : ((m_boldBox.isSelected() ? Font.BOLD
					: 0) + (m_italicBox.isSelected() ? Font.ITALIC : 0));
