...
public class EditorActionManager extends ActionManager {
	...
	public EditorActionManager(JFrame app, FileActionListener agent) {
		...
		m_actions.put(A_J_A_P, new AlignmentAction(A_J_A_P, StyleConstants.ALIGN_JUSTIFIED));
		m_actions.put(A_L_A_P, new AlignmentAction(A_L_A_P, StyleConstants.ALIGN_LEFT));
		m_actions.put(A_R_A_P, new AlignmentAction(A_R_A_P, StyleConstants.ALIGN_RIGHT));
		m_actions.put(B_A_P, new BeepAction());
		m_actions.put(BD_A_P, new BoldAction());
		m_actions.put(C_C_A_P, new ColourChooserAction(C_C_A_P, app));
		m_actions.put(CO_A_P, new CopyAction());
		m_actions.put(C_A_P, new CutAction());
		m_actions.put(F_C_A_P, new FontChooserAction(F_C_A_P, app));
		m_actions.put(IT_A_P, new ItalicAction());
		m_actions.put(F_N_A_P, new NewAction(F_N_A_P, app, agent));
		m_actions.put(F_O_A_P, new OpenAction(F_O_A_P, app, agent));
	...
	private final class ColourChooserAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			Editor editor = EditorActionManager.getActiveEditor();
			if (editor != null) {
				Color fg = JColorChooser.showDialog(m_parent, "Choose a colour...", null);
	...
	private final class ForegroundAction extends AbstractAction {
		...
		public ForegroundAction(String nm, Color fg) {
			// super(nm, new
			// ImageIcon(EditorActionManager.instance().create16x16ColourRec(c,
			// fg)));
			super(nm);
			m_name = nm;
			m_fg = fg;
	...
	private final class BoldAction extends AbstractAction {
		public BoldAction() {
			super(BD_A_P);
			putValue(Action.SMALL_ICON, new ImageIcon(Loader.load("bold_action16.gif")));
		}
	...
	private final class ItalicAction extends AbstractAction {
		public ItalicAction() {
			super(IT_A_P);
			putValue(Action.SMALL_ICON, new ImageIcon(Loader.load("italic_action16.gif")));
		}
	...
	private final class UnderlineAction extends AbstractAction {
		public UnderlineAction() {
			super(U_A_P);
			putValue(Action.SMALL_ICON, new ImageIcon(Loader.load("underline_action16.gif")));
		}
	...
	private final class StrikeThroughAction extends AbstractAction {
		public StrikeThroughAction() {
			super(STR_A_P);
			putValue(Action.SMALL_ICON, new ImageIcon(Loader.load("strikethrough_action16.gif")));
		}

		public void actionPerformed(ActionEvent e) {
			Editor editor = EditorActionManager.getActiveEditor();
			if (editor != null) {
				MutableAttributeSet attr = EditorActionManager.getActiveEditor()
						.getInputAttributes();
	...
	private static final class CutAction extends AbstractAction {
		public CutAction() {
			super(C_A_P);
			putValue(Action.SMALL_ICON, new ImageIcon(Loader.load("cut_action16.gif")));
		}
	...
	private static final class CopyAction extends AbstractAction {
		public CopyAction() {
			super(CO_A_P);
			putValue(Action.SMALL_ICON, new ImageIcon(Loader.load("copy_action16.gif")));
		}
	...
	private static final class PasteAction extends AbstractAction {
		public PasteAction() {
			super(P_A_P);
			putValue(Action.SMALL_ICON, new ImageIcon(Loader.load("paste_action16.gif")));
		}
	...
	protected static final class UndoAction extends AbstractAction {
		protected UndoAction() {
			super(UDO_A_AP);
			putValue(Action.SMALL_ICON, new ImageIcon(Loader.load("undo_action16.gif")));
		}
	...
	private static final class RedoAction extends AbstractAction {
		protected RedoAction() {
			super(RDO_A_P);
			putValue(Action.SMALL_ICON, new ImageIcon(Loader.load("redo_action16.gif")));
		}
	...
	private static final class NewAction extends AbstractAction {
		...
		public NewAction(String name, JFrame component, FileActionListener agent) {
			super(name);
			m_listener = agent;
			putValue(Action.SMALL_ICON, new ImageIcon(Loader.load("new_document16.gif")));
		}
	 ...
	 * the name is found in the list, it is the only one disabled. Otherwise,
	 * all actions that contain the pattern will be enabled/disabled.
	 */
	...
	protected final void setCharacterAttributes(JEditorPane editor, AttributeSet attr,
		...
		if (p0 != p1) {
			...
			if (doc instanceof StyledDocument) {
				((StyledDocument) doc).setCharacterAttributes(p0, p1 - p0, attr, replace);
			}
		} else {
	...
	/**
	 * Applies the given attributes to paragraphs. If there is a selection, the
	 * attributes are applied to the paragraphs that intersect the selection. if
	 * there is no selection, the attributes are applied to the paragraph at the
