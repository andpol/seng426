 ...
 */
public final class DocumentManager extends DefaultDesktopManager implements DesktopManager,
		FileActionListener {
	...
	public void editorOpen() {
		...
		filter = new DynamicFileFilter("java c cc cpp h txt text", "filter.plain.label");
	...
	public void editorOpenAs() {
		...
		FileFilter plainFilter = new DynamicFileFilter("*", "filter.any.plain.label");
		chooser.addChoosableFileFilter(plainFilter);
		FileFilter rtfFilter = new DynamicFileFilter("*", "filter.any.rtf.label");
		chooser.addChoosableFileFilter(rtfFilter);
		FileFilter htmlFilter = new DynamicFileFilter("*", "filter.any.html.label");
		...
		if (fileToOpen != null) {
			...
			try {
				DocumentFrame df = createDocumentFrame(null, fileToOpen.getName(), mime);
	...
	public void editorRevert(Editor editor) {
		...
		if (editor.getFile() == null) {
			...
			JOptionPane.showMessageDialog(p, msg, "Bad State", JOptionPane.ERROR_MESSAGE);
		} else {
			String msg = JWP.getMessage("dialog.revert.warning.1");
			String query = JWP.getMessage("dialog.revert.query");
			int answer = JOptionPane.showConfirmDialog(getParent(), msg, query,
	...
	public void editorSave(Editor editor) {
		...
		try {
			if (editor.getFile() == null) {
				JFileChooser chooser = new JFileChooser(JWP.getMessage("dialog.save.title"));
				...
				if (!fileToOpen.getName().startsWith("*")) {
					if (!ct.equals(mime)) {
						...
						int answer = JOptionPane.showConfirmDialog(p, msg, query,
								JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE);
						if (answer == JOptionPane.YES_OPTION) {
	...
	protected final DocumentFrame createDocumentFrame(File file) {
		return createDocumentFrame(file, ndt + p.getDocumentNumber(),
				ry.getString("MAIN", "default.content.type"));
	}

	protected final DocumentFrame createDocumentFrame(String contentType) {
		return createDocumentFrame(null, ndt + p.getDocumentNumber(), contentType);
	}

	protected final DocumentFrame createDocumentFrame() {
		return createDocumentFrame(null, ndt + (p.getDocumentNumber() + 1),
				ry.getString("MAIN", "default.content.type"));
	}
	...
	protected final void switchedDocument(DocumentFrame frame, boolean textSelected) {
	...
	protected final void cascade(DocumentFrame dframe) {
		...
		JWP.getDesktop().getDesktopManager()
				.setBoundsForFrame(dframe, nextX, nextY, targetWidth, targetHeight);
	}
	...
	protected final void cascadeAll() {
		...
		for (int i = 0; i < comps.length; i++) {
			if (comps[i] instanceof JInternalFrame && comps[i].isVisible()
				...
				JWP.getDesktop()
						.getDesktopManager()
						.setBoundsForFrame((JComponent) comps[i], nextX, nextY, targetWidth,
								targetHeight);
		...
		if (count != 0) {
			...
			for (int i = comps.length - 1; i >= 0; i--) {
				...
				if (comp instanceof JInternalFrame && comp.isVisible()
					...
					dm.setBoundsForFrame((JComponent) comp, x, y, columnWidth, rowHeight);
					y += rowHeight;
					if (--thisRow == 0) {
						y = 0;
						x += columnWidth;
	...
	private void showException(Exception ex) {
		...
		JOptionPane.showMessageDialog(p, msg, "Exception", JOptionPane.ERROR_MESSAGE);
	}
