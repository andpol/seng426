...
final class RegistryImpl extends Registry implements Serializable {
	...
	/**
	 * Test if the in-memory Registry is in sync with the stored version. This
	 * occurs when the contents of the registry have been changed, and are not
	 * stored. Calling commit() will sync the Registry with the stored version.
	 * 
	...
	private int[] stringToVersion(String str) throws RegistryFormatException {
		...
		if (str.equalsIgnoreCase("unspecified")) {
			...
			int i, pos, pos2;
			for (i = 0, pos = 0; (pos = str.indexOf('.', pos)) != -1; i++, pos++) {
				/**/
			}
			int[] version = new int[i + 1];
			try {
				for (i = 0, pos2 = 0; (pos = str.indexOf('.', pos2)) != -1; i++, pos2 = pos + 1) {
	...
	private String getBasicProperty(String group, String key, int type) {
		...
		if (rg.getPropertyType(key) != type) {
			throw new RegistryTypeException("property has wrong type", group, key,
					rg.getPropertyType(key), type);
		}
		return res;
	}
